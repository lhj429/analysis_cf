...
# pelicana
pelicana_table = pd.DataFrame.from_csv('__result__/crawling/pelicana_table.csv', encoding='utf-8', index_col=0, header=0).fillna('')    #  fillna : NaN을 특정 값으로 대체
pelicana_table = pelicana_table[pelicana_table.sido != '']      # row filtering (null이 아닌 값만)
pelicana_table = pelicana_table[pelicana_table.gungu != '']
pelicana = pelicana_table.apply(lambda r : str(r['sido']) + ' ' + str(r['gungu']), axis=1).value_counts()   # 'SIDO GUNGU'별 매장수

# nene
nene_table = pd.DataFrame.from_csv('__result__/crawling/nene_table.csv', encoding='utf-8', index_col=0, header=0).fillna('')
...
# showmap(chicken_merge, 'kyochon', '교촌 매장 분포', 'Reds')
# showmap(chicken_merge, 'goobne', '굽네 매장 분포', 'Oranges')
